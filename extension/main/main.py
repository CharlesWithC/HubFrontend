...
async def patchConfig(domain: str, request: Request, response: Response, authorization: str = Header(None)):
    ...
    try:
        if "logo_url" in data["config"].keys() and data["config"]["logo_url"].replace(" ", "") != "":
            resp = await arequests.head(data["config"]["logo_url"], headers=headers)
            if resp.status_code == 200:
                fs = int(resp.headers['Content-Length'])
                if fs > 1024 * 2048:
                    response.status_code = 400
                    return {"error": "Logo must not be larger than 2MB"}
                resp = await arequests.get(data["config"]["logo_url"], headers=headers)
                if resp.status_code == 200:
    ...
    try:
        if "banner_url" in data["config"].keys() and data["config"]["banner_url"].replace(" ", "") != "":
            resp = await arequests.head(data["config"]["banner_url"], headers=headers)
            if resp.status_code == 200:
                fs = int(resp.headers['Content-Length'])
                if fs > 1024 * 2048:
                    response.status_code = 400
                    return {"error": "Banner must not be larger than 2MB"}
                resp = await arequests.get(data["config"]["banner_url"], headers=headers)
                if resp.status_code == 200:
    ...
    try:
        if "bgimage_url" in data["config"].keys() and data["config"]["bgimage_url"].replace(" ", "") != "":
            resp = await arequests.head(data["config"]["bgimage_url"], headers=headers)
            if resp.status_code == 200:
                fs = int(resp.headers['Content-Length'])
                if fs > 1024 * 2048:
                    response.status_code = 400
                    return {"error": "Banner must not be larger than 2MB"}
                resp = await arequests.get(data["config"]["bgimage_url"], headers=headers)
                if resp.status_code == 200:
