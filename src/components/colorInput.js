...
const ColorInput = ({ color, onChange, hideDefault = false, disableDefault = false, disableCustom = false, defaultTooltip = "Default Color", customTooltip = "Custom Color", boxWrapper = true }) => {
  ...
  let content = (
    <>
      {!hideDefault && (
        <Tooltip placement="bottom" arrow title={defaultTooltip} PopperProps={{ modifiers: [{ name: "offset", options: { offset: [0, -10] } }] }}>
          <Box
            width="120px"
            height="60px"
            bgcolor="#777777"
            ...
            style={{ cursor: !disableDefault ? "pointer" : undefined, opacity: !disableDefault ? 1 : 0.6 }}>
            {customColor === "/" && !disableCustom && <CheckRounded />}
          </Box>
        </Tooltip>
      )}
      <Tooltip placement="bottom" arrow title={customTooltip} PopperProps={{ modifiers: [{ name: "offset", options: { offset: [0, -10] } }] }}>
        <Box width="120px" height="60px" bgcolor={color} onClick={handleClick} p={1} m={1} position="relative" borderRadius="5px" style={{ cursor: !disableCustom ? "pointer" : undefined, border: !isValidHexColor(color) ? `0.5px solid ${theme.palette.text.primary}88` : "none", opacity: !disableCustom ? 1 : 0.6 }}>
          ...
          {isValidHexColor(color) && (
            <Box display="flex" justifyContent="center" alignItems="center" style={{ height: "100%" }}>
              {!hideDefault && !disableDefault && <CheckRounded />}
            </Box>
          )}
        </Box>
      </Tooltip>
