...
function tabBtnProps(index, current, theme) {
    ...
    return {
        ...
        style: {
            color: current === index ? theme.palette.info.main : "inherit",
        },
    };
}
function TabPanel(props) {
    ...
    return (
        <div role="tabpanel" hidden={value !== index} id={`user-popover-${index}`} aria-labelledby={`user-popover-${index}`} {...other}>
...
function GetActivity(activity) {
    if (activity.status === "offline") {
        if (activity.last_seen !== -1)
            return (
                <>
                    {tr("offline_last_seen")}
                    <TimeAgo key={`${+new Date()}`} timestamp={activity.last_seen * 1000} lower={true} />
                </>
            );
...
const UserCard = props => {
    let { uid, userid, discordid, name, bio, note, global_note, avatar, email, steamid, truckersmpid, roles, tracker, ban, size, useChip, onDelete, textOnly, style, showProfileModal, onProfileModalClose } = {
        uid: -1,
        userid: -1,
        discordid: 0,
        name: "",
    ...
    const loadStats = useCallback(async () => {
        ...
        const [_tmp, _chart, _overall, _details, _point, _dlogList] = await makeRequestsAuto([
            {
                url: `https://config.chub.page/truckersmp?mpid=${truckersmpidRef.current}`,
                auth: false,
            },
            {
                url: `${vars.dhpath}/dlog/statistics/chart?userid=${userid}&ranges=7&interval=86400&sum_up=false&before=` + getTodayUTC() / 1000,
                auth: true,
            },
            {
                url: `${vars.dhpath}/dlog/statistics/summary?userid=${userid}`,
                auth: true,
            },
            {
                url: `${vars.dhpath}/dlog/statistics/details?userid=${userid}`,
                auth: true,
            },
            {
                url: `${vars.dhpath}/dlog/leaderboard?userids=${userid}`,
                auth: true,
            },
            {
                url: `${vars.dhpath}/dlog/list?userid=${userid}&page=${dlogPage}&page_size=${dlogPageSize}`,
                auth: "prefer",
            },
        ]);
            ...
            if (_dlogList.list[i].division.divisionid !== undefined) {
                divisionCheckmark = (
                    <Tooltip
                        placement="top"
                        arrow
                        title={tr("validated_division_delivery")}
                        PopperProps={{
                            modifiers: [
                                {
                                    name: "offset",
    ...
    useEffect(() => {
        if (chartStats === null && (ctxAction === "show-profile" || showProfileModal === 2)) loadStats();
    ...
    useEffect(() => {
        ...
        if (ctxAction === "show-profile" || showProfileModal === 2) doLoad();
    ...
    const updateUserInfo = useCallback(async () => {
        ...
        let resp = await axios({
            url: `${vars.dhpath}/user/profile?uid=${uid}`,
            method: "GET",
            headers: {
        ...
        if (resp.status === 200) {
            ...
            setNewConnections({
                ...
                discordid: resp.data.discordid,
                steamid: resp.data.steamid,
                truckersmpid: resp.data.truckersmpid,
            });
        ...
        const userUpdated = e => {
            if (e.detail !== undefined && e.detail.uid === uidRef.current) {
                ...
                setNewConnections({
                    ...
                    truckersmpid: user.truckersmpid,
                });
    ...
    const updateProfile = useCallback(
        async (sync_to = undefined) => {
            ...
            let resp = await axios({
                url: `${vars.dhpath}/user/profile?uid=${uid}${sync_to}`,
                method: "PATCH",
                data: newProfile,
                headers: {
                    Authorization: `Bearer ${getAuthToken()}`,
    ...
    const updateGlobalNote = useCallback(async () => {
        setDialogBtnDisabled(true);
        let resp = await axios({
            url: `${vars.dhpath}/user/${uid}/note/global`,
            method: "PATCH",
            data: {
                note: newGlobalNote,
            },
    ...
    const updateRoles = useCallback(async () => {
        setDialogBtnDisabled(true);
        let resp = await axios({
            url: `${vars.dhpath}/member/${useridRef.current}/roles`,
            method: "PATCH",
            data: {
    ...
    const updatePoints = useCallback(async () => {
        setDialogBtnDisabled(true);
        let resp = await axios({
            url: `${vars.dhpath}/member/${useridRef.current}/points`,
            method: "PATCH",
            data: {
                distance: newPoints.distance,
                bonus: newPoints.bonus,
            },
            headers: {
                Authorization: `Bearer ${getAuthToken()}`,
            },
    ...
    const switchTracker = useCallback(async () => {
        setDialogBtnDisabled(true);
        let resp = await axios({
            url: `${vars.dhpath}/user/tracker/switch?uid=${uid}`,
            data: {
                tracker: trackerInUse,
            },
            method: "POST",
            headers: {
                Authorization: `Bearer ${getAuthToken()}`,
    ...
    const acceptUser = useCallback(async () => {
        setDialogBtnDisabled(true);
        let resp = await axios({
            url: `${vars.dhpath}/user/${uid}/accept`,
            data: {
                tracker: trackerInUse,
            },
            method: "POST",
    ...
    const updateConnections = useCallback(
        async (action = "update", connection = "") => {
            ...
            if (action === "update") {
                ...
                resp = await axios({
                    url: `${vars.dhpath}/user/${uid}/connections`,
                    method: "PATCH",
                    data: processedNC,
                    headers: {
                        Authorization: `Bearer ${getAuthToken()}`,
                ...
                resp = await axios({
                    url: `${vars.dhpath}/user/${uid}/connections/${connection}`,
                    method: "DELETE",
                    headers: {
                        Authorization: `Bearer ${getAuthToken()}`,
                    },
    ...
    const disableMFA = useCallback(async () => {
        setDialogBtnDisabled(true);
        let resp = await axios({
            url: `${vars.dhpath}/user/mfa/disable?uid=${uid}`,
            data: {
                otp: otp,
            },
            method: "POST",
    ...
    const dismissMember = useCallback(async () => {
        setDialogBtnDisabled(true);
        let resp = await axios({
            url: `${vars.dhpath}/member/${useridRef.current}/dismiss`,
            method: "POST",
            headers: {
                Authorization: `Bearer ${getAuthToken()}`,
            },
    ...
    const putBan = useCallback(async () => {
        ...
        let meta = {
            ...removeNUEValues({
                ...
                steamid: steamid,
                truckersmpid: truckersmpid,
                expire: newBan.expire,
            }),
            reason: newBan.reason,
        };
        let resp = await axios({
            url: `${vars.dhpath}/user/ban`,
            method: "PUT",
            data: meta,
            headers: {
                Authorization: `Bearer ${getAuthToken()}`,
    ...
    let profileModal = (
        <Dialog
            ...
            <Card
                sx={{
                    padding: "5px",
                    backgroundImage: `linear-gradient(${profile_background[0]}, ${profile_background[1]})`,
                }}>
                {!vars.userSettings.data_saver && (
                    <CardMedia
                        ref={bannerRef}
                        component="img"
                ...
                <CardContent
                    sx={{
                        padding: "10px",
                        backgroundImage: `linear-gradient(${PROFILE_COLOR[theme.mode].paper}A0, ${PROFILE_COLOR[theme.mode].paper}E0)`,
                        borderRadius: "0 0 5px 5px",
                    }}>
                    <CardContent
                        sx={{
                            padding: "10px",
                            backgroundImage: `linear-gradient(${PROFILE_COLOR[theme.mode].paper}E0, ${PROFILE_COLOR[theme.mode].paper}E0)`,
                            borderRadius: "5px",
                        }}>
                        <div>
                            <div
                                ...
                                <Typography
                                    ...
                                    {useridRef.current !== null && useridRef.current !== undefined && useridRef.current >= 0 && (
                                        <Tooltip
                                            placement="top"
                                            arrow
                                            title={tr("user_id")}
                                    ...
                                    {showProfileModal !== 2 && (uid === vars.userInfo.uid || (uid !== -1 && checkUserPerm(["administrator", "manage_profiles"]))) && (
                                        <>
                                            &nbsp;
                                            <IconButton
                                                size="small"
                            ...
                            {uid === vars.userInfo.uid && !customizeProfileAck && vars.userConfig[vars.userInfo.uid] === undefined && (
                                <Typography
                                    variant="body2"
                                    sx={{
                                        color: theme.palette.info.main,
                        ...
                        <SimpleBar
                            style={{
                                height: `calc(100vh - 310px - ${bannerRef.current !== null && bannerRef.current.height !== 0 ? bannerRef.current.height : 104.117}px)`,
                            }}>
                            <TabPanel value={tab} index={0}>
                                                ...
                                                avatar={
                                                    <div
                                                        style={{
                                                            ...
                                                            width: "12px",
                                                            height: "12px",
                                                            backgroundColor: vars.roles[role] !== undefined && vars.roles[role].color !== undefined ? vars.roles[role].color : "#777777",
                                                            borderRadius: "100%",
                                                        }}
                                                    />
                                                }
                                ...
                                <Box
                                    ...
                                    <TextField value={newNote} onChange={e => setNewNote(e.target.value)} fullWidth multiline size="small" />
                                </Box>
                                <Divider />
                                <Box
                                    sx={{
                                    ...
                                    <Grid container spacing={2}>
                                        {emailRef.current !== undefined && emailRef.current !== null && (
                                            <Grid item xs={12} sm={12} md={6} lg={6}>
                                                <a href={`mailto:${emailRef.current}`} target="_blank" rel="noreferrer">
                                                    <Chip
                                                        avatar={
                                                            <Tooltip
                                                                placement="top"
                                        ...
                                        {discordidRef.current !== undefined && discordidRef.current !== null && (
                                            <Grid item xs={12} sm={12} md={6} lg={6}>
                                                <a href={`https://discord.com/users/${discordidRef.current}`} target="_blank" rel="noreferrer">
                                                    <Chip
                                                        avatar={
                                                            <Tooltip
                                                                placement="top"
                                        ...
                                        {steamidRef.current !== undefined && steamidRef.current !== null && (
                                            <Grid item xs={12} sm={12} md={6} lg={6}>
                                                <a href={`https://steamcommunity.com/profiles/${steamidRef.current}`} target="_blank" rel="noreferrer">
                                                    <Chip
                                                        avatar={
                                                            <Tooltip
                                                                placement="top"
                                        ...
                                        {truckersmpidRef.current !== undefined && truckersmpidRef.current !== null && (
                                            <Grid item xs={12} sm={12} md={6} lg={6}>
                                                <a href={`https://truckersmp.com/user/${truckersmpidRef.current}`} target="_blank" rel="noreferrer">
                                                    <Chip
                                                        avatar={
                                                            <Tooltip
                                                                placement="top"
                                                            ...
                                                            <>
                                                                {tmpLastOnline === null ? (
                                                                    ...
                                                                    <Tooltip
                                                                        ...
                                                                        title={
                                                                            <>
                                                                                <>{tr("last_seen")}</>: <TimeAgo key={`${+new Date()}`} timestamp={tmpLastOnline * 1000} />
                                                                            </>
                                                                        }
                                                                        PopperProps={{
                                                                            modifiers: [
                                        ...
                                        {detailStats !== null && detailStats.truck.length >= 1 && detailStats.cargo.length >= 1 && (
                                            <>
                                                <Grid item xs={6} sm={6} md={6} lg={6}>
                                                    ...
                                                    <Typography variant="body2">
                                                        {detailStats.truck[0].name} ({detailStats.truck[0].count} <>times</>)
                                                    </Typography>
                                                </Grid>
                                                <Grid item xs={6} sm={6} md={6} lg={6}>
                                                    <Typography
                                                    ...
                                                    <Typography variant="body2">
                                                        {detailStats.cargo[0].name} ({detailStats.cargo[0].count} <>times</>)
                                                    </Typography>
                                                </Grid>
                                            </>
                                        )}
    ...
    if (uid === null)
        return (
            <>
                <Avatar
                    src={!vars.userSettings.data_saver ? avatarRef.current : ""}
                    style={{
                        width: `${size}px`,
                        height: `${size}px`,
        ...
        <>
            ...
            {showContextMenu && (
                <Menu
                    ...
                    {(uid === vars.userInfo.uid || (uid !== -1 && checkUserPerm(["administrator", "manage_profiles"]))) && (
                        <MenuItem
                            onClick={e => {
                    ...
                    <Divider />
                    {checkUserPerm(["administrator", "update_global_note"]) && (
                        <MenuItem
                            onClick={e => {
                    ...
                    {userid !== null && userid >= 0 && checkUserPerm(["administrator", "manage_divisions", "update_roles"]) && (
                        <MenuItem
                            onClick={e => {
                    ...
                    {userid !== null && userid >= 0 && checkUserPerm(["administrator", "update_points"]) && (
                        <MenuItem
                            onClick={e => {
                    ...
                    {(userid === null || userid < 0) && ban === null && checkUserPerm(["administrator", "accept_members"]) && (
                        <MenuItem
                            sx={{
                    ...
                    {checkUserPerm(["administrator", "update_connections"]) && (
                        <MenuItem
                            sx={{
                    ...
                    {checkUserPerm(["administrator", "disable_mfa"]) && (
                        <MenuItem
                            sx={{
                                color: theme.palette.warning.main,
                            }}
                    ...
                    {(userid === null || userid < 0) && ban === null && checkUserPerm(["administrator", "ban_users"]) && (
                        <MenuItem
                            sx={{
                    ...
                    {userid !== null && userid >= 0 && checkUserPerm(["administrator", "dismiss_members"]) && (
                        <MenuItem
                            sx={{
                                color: theme.palette.error.main,
                            }}
                    ...
                    {checkUserPerm(["administrator", "delete_users"]) && (
                        <MenuItem
                            sx={{
                                color: theme.palette.error.main,
                            }}
                ...
                {ctxAction === "update-profile" && (
                    <Dialog
                        ...
                        <DialogTitle>
                            {tr("update_profile")}
                            <>|</>
                            {nameRef.current} ({userid !== null ? tr("user_id") + ": " + useridRef.current + " / " : ""}
                            <>UID</>: {uid})
                        </DialogTitle>
                        <DialogContent>
                ...
                {ctxAction === "update-roles" && (
                    <Dialog
                        ...
                        <DialogTitle>
                            {tr("update_roles")}
                            <>|</>
                            {nameRef.current} (<>{tr("user_id")}</>: {useridRef.current})
                        </DialogTitle>
                        <DialogContent>
                        ...
                        <DialogContent>
                            ...
                            <FormControl
                                ...
                                <TextField
                                    ...
                                    size="small"
                                    value={trackerInUse}
                                    onChange={e => setTrackerInUse(e.target.value)}
                                    sx={{
                                        marginTop: "6px",
                                        height: "30px",
                                    }}>
                            ...
                            <Box display="flex" alignItems="center">
                                ...
                                <Typography
                                    ...
                                    style={{
                                        fontSize: "0.8em",
                                        marginLeft: "8px",
                                        color: roleHistory === null ? theme.palette.error.main : roleHistory !== undefined ? theme.palette.success.main : theme.palette.info.main,
                                    }}>
                                    {roleHistory === null ? `Invisible` : roleHistory !== undefined ? `Visible` : `Loading`}
                                </Typography>
                            </Box>
                            {roleHistory !== undefined &&
                                roleHistory !== null &&
                                roleHistory.map((history, idx) => (
                                    <>
                                        {idx !== 0 && (
                                            <Divider
                                                sx={{
                                        ...
                                        {history.added_roles.map(role => (
                                            <Typography
                                                ...
                                                + {vars.roles[role] !== undefined ? vars.roles[role].name : `Unknown Role (${role})`}
                                            </Typography>
                                        ))}
                                        ...
                                        <Typography
                                            key={`history-${idx}-time`}
                                            variant="body2"
                                            sx={{
                                                color: theme.palette.text.secondary,
                                            }}>
                                            <TimeAgo key={`${+new Date()}`} timestamp={history.timestamp * 1000} />
                                        </Typography>
                                    </>
                                ))}
                            ...
                            <Box
                                ...
                                <Typography
                                    ...
                                    style={{
                                        fontSize: "0.8em",
                                        marginLeft: "8px",
                                        color: banHistory === null ? theme.palette.error.main : banHistory !== undefined ? theme.palette.success.main : theme.palette.info.main,
                                    }}>
                                    {banHistory === null ? `Invisible` : banHistory !== undefined ? `Visible` : `Loading`}
                                </Typography>
                            </Box>
                            {banHistory !== undefined &&
                                banHistory !== null &&
                                banHistory.map((history, idx) => (
                                    <>
                                        {idx !== 0 && (
                                            <Divider
                                                sx={{
                                        ...
                                        <Typography
                                            ...
                                            <>{tr("expiry")}</>: {getFormattedDate(new Date(history.expire_timestamp * 1000))}
                                        </Typography>
                                    </>
                                ))}
