...
const SideBar = props => {
  ...
  const menu = useMemo(() => {
    ...
    if (curUID === null) {
      ...
      menu = [
        ["overview", "statistics", "announcement", "downloads", "poll", "task", "gallery"],
        ["live_map", "delivery", "challenge", "division", "division_pending", "event", "economy"],
  ...
  const sidebar = (
    <SimpleBar key="sidebar-simplebar" style={{ maxHeight: `100vh`, height: `100vh` }}>
              ...
              <Link key={`navlink-${menuID}-${btnIndex}`} to={`${menuRoute[menuID]}`}>
                <ListItem key={`navbtn-${menuID}-${btnIndex}`} disablePadding>
                  <ListItemButton selected={selectedIndex === subIndex * 10 + btnIndex} onClick={event => handleListItemClick(event, subIndex * 10 + btnIndex)}>
                    <ListItemIcon sx={{ minWidth: "40px" }}>{menuIcon[menuID]}</ListItemIcon>
  ...
  return (
    <Box
      ...
      <Drawer
        ...
        <div style={{ overflow: "hidden" }}>{sidebar}</div>
      </Drawer>
      <Drawer
        variant="permanent"
        sx={{
