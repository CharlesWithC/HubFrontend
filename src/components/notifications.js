...
const NotificationsPopover = () => {
    ...
    const loadNotifications = useCallback(async () => {
        ...
        try {
            const resp = await axios({
                url: `${vars.dhpath}/user/notification/list`,
                params: {
                    page_size: 250,
                },
                method: "GET",
        ...
        try {
            const resp = await axios({
                url: `${vars.dhpath}/user/notification/list?status=0`,
                method: "GET",
                headers: {
                    Authorization: `Bearer ${bearerToken}`,
                },
    ...
    const handleAllRead = async () => {
        ...
        try {
            const resp = await axios({
                url: `${vars.dhpath}/user/notification/all/status/1`,
                method: "PATCH",
                headers: {
                    Authorization: `Bearer ${bearerToken}`,
                },
                    ...
                    {notifications !== null && (
                        ...
                        >
                            {notifications.map(({ id, message, timestamp, read }) => (
                                ...
                                >
                                    <ListItemText
                                        primary={<ReactMarkdown>{message}</ReactMarkdown>}
                                        secondary={timestamp !== null ? new Date(timestamp * 1000).toLocaleString() : ""}
                                        primaryTypographyProps={{
                                            style: {
                                                color: read === true ? "grey" : null,
                                            },
                                        }}
                            ...
                            >
                                <ListItemText
                                    primary={tr("last_250_notifications_shown")}
                                    primaryTypographyProps={{
                                        style: {
                                            color: "grey",
                                            fontSize: "0.8em",
