...
const UserSelect = ({ label, users, onUpdate, isMulti = true, includeCompany = false, includeBlackhole = false, limit = undefined, style = {}, userList: inputUserList = undefined, disabled = false, allowSelectAll = false, allowDeselect = false }) => {
  ...
  return (
    <div style={style}>
      ...
      <Select isMulti={isMulti} name="colors" options={options} className="basic-multi-select" classNamePrefix="select" styles={customSelectStyles(theme)} value={selectedUsers} onChange={handleInputChange} menuPortalTarget={document.body} isDisabled={disabled} components={{ DropdownIndicator }} />
    </div>
  );
};
