...
const FreightMaster = () => {
    ...
    const doLoad = useCallback(async () => {
        ...
        if (!positionLoaded.current) {
            [position, lb] = await makeRequestsWithAuth([`https://config.chub.page/freightmaster/position?abbr=${webConfig.abbr}&uid=${curUID}`, `https://config.chub.page/freightmaster/${fMode}?abbr=${webConfig.abbr}&page=${page}&page_size=${pageSize}`]);
            ...
            [lb] = await makeRequestsWithAuth([`https://config.chub.page/freightmaster/${fMode}?abbr=${webConfig.abbr}&page=${page}&page_size=${pageSize}`]);
        }
            ...
            for (let i = 0; i < (lb.list ? lb.list.length : 0); i++) {
                newlb.push({
                    ...
                    vtc: (
                        <>
                            <Avatar
                                src={!userSettings.data_saver ? "https://cdn.chub.page/assets/" + lb.list[i].abbr + "/logo.png" : ""}
                                style={{
                                    width: `20px`,
                                    height: `20px`,
                            ...
                            />
                            <span key={`user-${Math.random()}`} style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", marginLeft: "2px" }}>
                                {lb.list[i].vtc}
                            </span>
                        </>
                    ),
                    ...
                    points: TSep(lb.list[i].point),
                    level: fMode === "d" ? parseInt(lb.list[i].point / D_LEVEL_POINT) + 1 : parseInt(lb.list[i].point / A_LEVEL_POINT + 1),
                });
            }
                ...
                <Card>
                    <CardContent>
                        <Typography variant="h5" fontWeight="bold" fontSize="30px">
                            {tr("rank")} {fMode === "d" ? rankd : ranka}
                        </Typography>
                        <Typography variant="body2" fontSize="25px">
                            {tr("points")} {TSep(fMode === "d" ? pointd : pointa)}
                        </Typography>
                        <Typography variant="body2" fontSize="25px">
                            {tr("level")} {fMode === "d" ? parseInt(pointd / D_LEVEL_POINT + 1) : parseInt(pointa / A_LEVEL_POINT + 1)}
                        </Typography>
                    </CardContent>
                </Card>
                <Card sx={{ mt: "10px" }}>
                    <CardContent>
                        ...
                        <Typography variant="body2" fontSize="15px" color="grey" sx={{ mb: "5px" }}>
                            {tr("only_applies_to_crossvtc_ranking")}
                        </Typography>
                        <Typography variant="body2" fontWeight="bold" fontSize="15px">
                            {tr("player_title")}
                        </Typography>
                        {fmRewards
                            .filter(reward => reward.reward_type === "title" && reward.active)
                            .map(reward => (
                                <Typography variant="body2" fontSize="15px">
                                    {reward.reward_value}
                                    {reward.qualify_type === "percentage" && (
                                        <>
                                            {" "}
                                            ({reward.finisher_reward ? `Finishes` : `Been`} {reward.qualify_value.substring(0, 2) == "==" ? "at" : "top"} {parseFloat(reward.qualify_value.substring(2)) * 100}%)
                                        </>
                                    )}
                                    {reward.qualify_type === "rank" && (
                                        <>
                                            {" "}
                                            ({reward.finisher_reward ? `Finishes` : `Been`} {reward.qualify_value.substring(0, 2) == "==" ? "at" : "top"} {reward.qualify_value.substring(2)})
                                        </>
                                    )}
