...
function ParseChallengeImage(challenges) {
...
const ChallengeCard = ({ challenge, onShowDetails, onUpdateDelivery, onEdit, onDelete }) => {
    ...
    const iconStyle = { borderRadius: '50%', padding: '2px' };
    ...
    let statusIcon = challenge.start_time * 1000 <= Date.now() && challenge.end_time * 1000 >= Date.now()
        ? <PlayCircleRounded sx={{ ...iconStyle, backgroundColor: theme.palette.success.main }} />
        : challenge.start_time * 1000 > Date.now()
            ? challenge.start_time * 1000 > Date.now() + 86400 ? <ScheduleRounded sx={{ ...iconStyle, backgroundColor: theme.palette.info.main }} /> :
                <HourglassBottomRounded sx={{ ...iconStyle, backgroundColor: theme.palette.info.main }} />
            : <StopCircleRounded sx={{ ...iconStyle, backgroundColor: theme.palette.error.main }} />;

    return (
        <Card>
            <CardMedia component="img" src={challenge.image} alt=" " />
            <LinearProgress variant="determinate" color="success" value={Math.min(parseInt(challenge.current_delivery_count / challenge.delivery_count * 100) + 1, 100)} sx={{ width: "100%" }} />
            <CardContent>
                <div style={{ marginBottom: "10px", display: 'flex', alignItems: "center" }}>
                    <Typography variant="h6" gutterBottom sx={{ flexGrow: 1 }}>
                        <div style={{ display: 'flex', alignItems: 'center' }}>
                            {statusIcon}&nbsp;
                            {completed && <>
                                <TaskAltRounded sx={{ ...iconStyle, backgroundColor: theme.palette.warning.main }} />&nbsp;
                            </>}
                            {qualified && <>
                                <DoneOutlineRounded sx={{ ...iconStyle, backgroundColor: theme.palette.success.main }} />&nbsp;
                            </>}
                            {!qualified && <>
                                <BlockRounded sx={{ ...iconStyle, backgroundColor: theme.palette.secondary.main }} />&nbsp;
                            </>}
                            {challenge.title}
                        </div>
                    </Typography>
...
const ChallengesMemo = memo(({ upcomingChallenges, setUpcomingChallenges, activeChallenges, setActiveChallenges, onShowDetails, onUpdateDelivery, onEdit, onDelete, doReload }) => {
    useEffect(() => {
        async function doLoad() {
            ...
            let [_upcomingChallenges, _activeChallenges] = await makeRequestsWithAuth(urls);
            setUpcomingChallenges(_upcomingChallenges.list);
            setActiveChallenges(_activeChallenges.list);

            const loadingEnd = new CustomEvent('loadingEnd', {});
            window.dispatchEvent(loadingEnd);
        }
        doLoad();
    }, [doReload, setUpcomingChallenges, setActiveChallenges]);

    return <>
        <Grid container spacing={2}>
            {upcomingChallenges.length !== 0 && <>
                <Grid item xs={upcomingChallenges.length === 2 ? 6 : 12}>
                    <ChallengeCard challenge={upcomingChallenges[0]} onShowDetails={onShowDetails} onUpdateDelivery={onUpdateDelivery} onEdit={onEdit} onDelete={onDelete} />
                </Grid>
                {upcomingChallenges.length === 2 && <Grid item xs={6}>
                    <ChallengeCard challenge={upcomingChallenges[1]} onShowDetails={onShowDetails} onUpdateDelivery={onUpdateDelivery} onEdit={onEdit} onDelete={onDelete} />
                </Grid>}
            </>}
...
const Challenges = () => {
    ...
    const [doReload, setDoReload] = useState(new Date());

    return <ChallengesMemo upcomingChallenges={upcomingChallenges} setUpcomingChallenges={setUpcomingChallenges} activeChallenges={activeChallenges} setActiveChallenges={setActiveChallenges} doReload={doReload} />;
};
