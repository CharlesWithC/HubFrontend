    ...
    useEffect(() => {
        async function doLoad() {
            ...
            if (!inited.current) {
                ...
                [_applications] = await makeRequestsAuto([{ url: `${apiPath}/applications/list?page=${page}&page_size=${pageSize}&${new URLSearchParams(processedParam).toString()}`, auth: true }]);
            }
            let newApplications = [];
                    ...
                    if (resp.status === 200) {
                        ...
                        Object.entries(resp.data.application).map(([_, answer]) => {
                            ...
                            if (matcha1) {
                                assignee = (
                                    <>
                                        Assigned to {users[matcha1[1]] !== undefined && <UserCard user={users[matcha1[1]]} />}
                        ...
                        <Card>
                            <CardContent>
                                <Typography variant="subtitle2" gutterBottom>
                                    {tr("recent")} {applicationTypes !== null && applicationTypes[recent[0].type] ? applicationTypes[recent[0].type].name : tr("unknown")} {tr("application")}
                                </Typography>
                    ...
                    {recent.length === 2 && (
                        <Grid
                            ...
                            <Card>
                                <CardContent>
                                    <Typography variant="subtitle2" gutterBottom>
                                        {tr("recent")} {applicationTypes !== null && applicationTypes[recent[1].type] ? applicationTypes[recent[1].type].name : tr("unknown")} {tr("application")}
                                    </Typography>
                                    <Typography variant="h5" component="div">
                    ...
                    <DialogContent sx={{ minWidth: "550px" }}>
                        {Object.entries(detailApp.application).map(([question, answer]) => {
                            ...
                            if (matchq) {
                                question = (
                                    <>
                                        Message from {membersMapping[matchq[2]] !== undefined && <UserCard user={membersMapping[matchq[2]]} />}
                        ...
                        <div style={{ display: detailApp.status !== 0 ? "none" : "block" }}>
                            ...
                            <TextField value={message} onChange={e => setMessage(e.target.value)} multiline rows="5" fullWidth />
                        </div>
                    </DialogContent>
                    <DialogActions>
                        <Box sx={{ padding: "10px" }}>
                            <Button
                                ...
                                sx={{ display: detailApp.status !== 0 ? "none" : "block" }}>
                                {tr("respond")}
                            </Button>
                        </Box>
                    </DialogActions>
